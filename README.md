...
Nothing is lost. Resolve the reported cause, then re-run `git stack merge --all`: it re-reads open PRs from GitHub, so already-merged ones drop out of the list and it resumes from the first one still open.
